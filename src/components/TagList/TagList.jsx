...
function TagList(props) {
    ...
    return (
        <div className="tag-wrapper">
            <input ref={inputRef} type="text" onChange={searchWithFuse} className="input-search-tags" placeholder="Search for your tags here..."></input>
            <ul className="tag-container">
                {filteredTags.length > 0 ? (
                    <>
                        {filteredTags.map((tag) => (
                            <li key={tag.id} id={tag.id} className="tag" onClick={handleClick}>
                                {props.isAdmin && (
